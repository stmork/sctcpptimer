...
namespace sc
{
	namespace timer
	{
		...
		private:
			struct TimeBased
			{
				std::weak_ptr<TimedInterface> handle;
				sc::eventid pt_evid;
				bool periodic;
				TimeBased() : handle(), pt_evid(0), periodic(false) {}
			};
			struct NonEmpty
			{
				sc::time time_ms;
				TimeBased time_event;
				std::weak_ptr<CycleBasedInterface> run_cycle_handle;
				NonEmpty() : time_ms(0), time_event(), run_cycle_handle() {}
			};
		public:
			struct TaskData
			{
				...
				TaskData(std::weak_ptr<TimedInterface> time_event_handle, sc::eventid pt_evid, sc::time time_ms, bool periodic) :
				...
				TaskData(std::weak_ptr<CycleBasedInterface> run_cycle_handle, sc::time time_ms) :
					type(RUNCYCLE_TASK)
				{
					get.time_ms = time_ms;
				...
				void reset()
				{
					...
					case TIME_EVENT_TASK:
						get.time_ms = 0;
						get.time_event.handle.reset();
						get.time_event.pt_evid = 0;
						get.time_event.periodic = false;
						break;
					case RUNCYCLE_TASK:
						get.time_ms = 0;
						get.run_cycle_handle.reset();
						break;
					default:
						return;
					}
				...
				virtual void execute()
				{
					...
					case TIME_EVENT_TASK:
						{
							// fire the event
							std::shared_ptr<TimedInterface> time_event_handle = get.time_event.handle.lock();
					...
					case RUNCYCLE_TASK:
						{
							std::shared_ptr<CycleBasedInterface> run_cycle_handle = get.run_cycle_handle.lock();
			...
			*/
			...
			{
			private:
				std::weak_ptr<TimedInterface> time_event_handle;
				sc::eventid pt_evid;
			public:
				MatchTimeEvent(std::weak_ptr<TimedInterface> time_event_handle_, sc::eventid pt_evid_): time_event_handle(time_event_handle_), pt_evid(pt_evid_) {}

				virtual bool match(const TimerTask & other)  override
				{
					return other.data.type == TaskData::TIME_EVENT_TASK && time_event_handle.lock() == other.data.get.time_event.handle.lock() && pt_evid == other.data.get.time_event.pt_evid;
				}
			...
			private:
				std::weak_ptr<CycleBasedInterface> run_cycle_handle;
			public:
				MatchRunCycleOf(std::weak_ptr<CycleBasedInterface> run_cycle_handle_): run_cycle_handle(run_cycle_handle_) {}

				virtual bool match(const TimerTask & other)  override
				{
					return other.data.type == TaskData::RUNCYCLE_TASK && run_cycle_handle.lock() == other.data.get.run_cycle_handle.lock();
				}
		...
		private:
			size_t length;
			std::unique_ptr<TimerTask[]> tasks;
		...
		public:
			...
			TimerService(std::unique_ptr<TimerTask[]> && tasks_, size_t length_) :
				length(length_), //
			 ...
			 */
			virtual void setTimer(std::shared_ptr<TimedInterface >statemachine_, sc::eventid event,
			 ...
			 */
			virtual void setRuncycleTimerFor(std::shared_ptr<CycleBasedInterface >statemachine, sc::time cycle_period);

			/*! Unset timers for running cycles of the given statemachine.
			 */
			virtual void unsetRuncycleTimerFor(std::shared_ptr<CycleBasedInterface >statemachine);
